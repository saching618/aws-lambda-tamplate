...
import { AUTH_PERMISSION, REDIS_DEFAULT_TTL } from '../constant';
...
export const handler: APIGatewayProxyHandler = async (event:any): Promise<any> => {
    ...
    try{
        ...
        if(key === "" || value === ""){
            ...
            return {
            statusCode: 401,
            body: 'Bad Request  - {key or value is missing}'
            };
        }
        await initRedis(requestId);
        const RedisObj = new RedisClient();
        console.log(requestId,`lambda Request data: ${JSON.stringify({data})}`);
        const response:any = await RedisObj.setValueWithTimeout(requestId, key, value, REDIS_DEFAULT_TTL);
